class IdentifiedResource
	attr_accessor :commodity, :grade_unit, :grade_factor, :commodity_unit, :commodity_factor, :ore_unit, :ore_factor,
		:year, :recorddate, :material, :reserves_ore, :reserves_commodity, :edr_ore, :edr_commodity, :dmp_ore, :dmp_commodity, :dms_ore, :dms_commodity, :ifr_ore, :ifr_commodity
	
  def initialize
    @reserves_commodity = 0.0
    @reserves_ore = 0.0
    @edr_commodity = 0.0
    @edr_ore = 0.0
    @dmp_commodity = 0.0
  ...
  def total_grade
    return total_commodity/total_ore rescue 0
  end

  def reserves_grade
    return reserves_commodity/reserves_ore rescue 0
  end
  ...
  def set_resources (resource, options={})
    ...
    resource.each do |r|
      ...
      self.reserves_ore += get_pvr(r, "edr")[:ore] + get_pbr(r, "edr")[:ore] + get_ppr(r, "edr")[:ore]
      self.reserves_commodity += get_pvr(r, "edr")[:ore] + get_pbr(r, "edr")[:ore] + get_ppr(r, "edr")[:commodity]
  ...
  def ifr (resource, ga_class)
    ...
    if resource.ifr && resource.resource_ifr
      if send(resource_class).include?(resource.ifr_class1)
        ...
        ore += resource.resource_ifr.to_f * ore_factors[resource.unit_quantity].to_f * (resource.ifr_pcnt1.to_f/100)
      end
      if send(resource_class).include?(resource.ifr_class2)
        ...
        ore += resource.resource_ifr * ore_factors[resource.unit_quantity] * (resource.ifr_pcnt2.to_f/100)
      end
    end
    return {:commodity => contained_metal,:ore=>ore}
  end
